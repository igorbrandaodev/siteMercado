...
export class LoginComponent implements OnInit {
    ...
    // Realiza a autenticação na base
    login() {
    ...
    // Realiza a autenticação na API da SiteMercado
    loginSiteMercado() {
        ...
        this.authService
            ...
            .subscribe((response) => {
                // Obtém o retorno
                let responseModel = response.body.valueOf();
                this.toastr.show(responseModel.toString());
            }, (error) => {
                console.log(error)
                this.toastr.error('Erro! Não foi possível se comunicar com a API.');
            });
    }
